...
export function Game({ initialPhase, initialState }: { initialPhase?: GamePhase; initialState?: Partial<GameState> }) {
  ...
  return (
    <main className={`game-shell phase-${state.phase}`}>
      ...
      {state.phase === 'start' && <section className="screen start-screen"><div className="start-orbit" aria-hidden="true"><span>Q2</span><span>Q1</span><span>Q3</span><span>Q4</span></div><div><h1>อาณาจักรสี่จตุภาค</h1><p className="screen-en">Four Quadrants Kingdom</p><p>ฟื้นฟูวงกลมหนึ่งหน่วยด้วยพลังของตำแหน่งและเครื่องหมาย</p><p className="screen-en">Restore the Unit Circle through position and signs.</p></div><button className="primary-button" onClick={() => dispatch({ type: 'START' })}>เริ่มการเดินทาง <span>Begin journey</span></button></section>}
