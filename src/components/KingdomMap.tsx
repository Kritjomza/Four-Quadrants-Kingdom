  ...
  return (
    <section className="screen map-screen">
      ...
      <div className="map-compass" aria-label="แผนที่สี่จตุภาค">
        {spatialOrder.map((quadrant) => GUIDED_LANDS.find((land) => land.quadrant === quadrant)!).map((land) => <div key={land.quadrant} className={`map-land land-${land.quadrant.toLowerCase()} ${fragments.includes(land.quadrant) ? 'done' : ''} ${land.quadrant === current.quadrant ? 'current' : ''}`}><LandArt quadrant={land.quadrant} /><b>{land.quadrant}</b><span>{land.name.th}</span><small>{fragments.includes(land.quadrant) ? 'เก็บชิ้นส่วนแล้ว ✓' : land.quadrant === current.quadrant ? 'จุดหมายถัดไป' : 'ยังไม่สำรวจ'}</small></div>)}
      </div>
